...
def is_tradeable(id):
    data = get_instrument_by_id(id)
    if data != None and data["tradeable"] and data["tradability"] == "tradable":
            return True
    else:
            return False
...
def get_instruments_by_symbols(inputSymbols, info=None, warnings=True):
...
def get_latest_price(inputSymbols, includeExtendedHours=False):
    ...
    prices = []
    for item in quote:
        if not includeExtendedHours or item['last_extended_hours_trade_price'] is None:
...
def get_quote_by_symbol(symbol, info=None):
    """
    Represents basic stock quote information
    ...
    return get_quote_by_id(helper.id_for_stock(symbol))
